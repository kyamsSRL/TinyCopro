...
export async function getMemberEmails(coproId: string) {
  const { data, error } = await supabase.rpc('get_member_emails', { p_copro_id: coproId });
  return { emails: (data as string[] | null) ?? [], error };
}

export async function updateMilliemes(membreId: string, milliemes: number) {
  const { error } = await supabase.rpc('update_membre_milliemes', { p_membre_id: membreId, p_milliemes: milliemes });
  return { error };
}

export async function transferRole(params: { fromMembreId: string; toMembreId: string }) {
  const { error } = await supabase.rpc('transfer_role', { p_from_membre_id: params.fromMembreId, p_to_membre_id: params.toMembreId });
  return { error };
}
